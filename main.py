...
class Server:
    def __init__(self):
        ...
        self.ServerName = "python db server"
    ...
    def ServerCommands(self, conn, addr):
        run = True
        while self.running:
            try:
                ...
                if command != "":
                    print(f"IP: {addr[0]}, PORT: {addr[1]}, command: {command}")
                else:
                    print(f"client has disconnected IP: {addr[0]}, PORT: {addr[1]}")
                    break
            except:
                pass
    ...
    def listening(self):
        while self.running:
            try:
                ...
                self.data_client.append([conn, addr])

                threading.Thread(target=lambda: self.ServerCommands(conn, addr)).start()
